  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {grows.map((grow) => (
          <GrowCard key={grow.id} grow={grow} onEdit={onEditGrow} />
        ))}
      </div>
    </section>
  );
...
function StatsPage({ grows }) {
  const stats = useMemo(() => {
    const allHarvests = grows.flatMap((grow) =>
      grow.harvests.map((h) => ({ ...h, species: grow.species })),
    );
    ...
    const statusTemplate = [
      { name: 'Harvested', value: 0 },
      { name: 'Contaminated', value: 0 },
      { name: 'Growing', value: 0 },
    ];
    const statusCounts = statusTemplate.map((item) => ({ ...item }));
    grows.forEach((grow) => {
      const match = statusCounts.find((item) => item.name === grow.status);
      if (match) match.value += 1;
      else statusCounts[2].value += 1;
    });

    const bestFlush = grows
      .map((grow) => {
        const byFlush = Object.values(
          grow.harvests.reduce((acc, item) => {
            const flushKey = `Flush ${item.flush}`;
            acc[flushKey] = acc[flushKey] || { flush: flushKey, yield: 0 };
            acc[flushKey].yield += Number(item.weight);
            return acc;
          }, {}),
        );

        const topFlush = byFlush.sort((a, b) => b.yield - a.yield)[0];
        return {
          name: grow.name,
          flush: topFlush?.flush || 'No harvest',
          yield: topFlush?.yield || 0,
        };
      })
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        <ChartPanel title="Success Rate (Harvested vs Contaminated vs Growing)">
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
        ...
        <ChartPanel title="Best Flush Performance">
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={stats.bestFlush}>
              <CartesianGrid strokeDasharray="3 3" stroke="#2c3932" />
              <XAxis dataKey="name" stroke="#ddd3c4" angle={-15} textAnchor="end" interval={0} height={64} />
              <YAxis stroke="#ddd3c4" />
              <Tooltip formatter={(value, _name, row) => [`${value} g`, row?.payload?.flush || 'No harvest']} />
